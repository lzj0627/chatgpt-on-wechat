...
class Apilot(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        if e_context["context"].type not in [
            ContextType.TEXT, ContextType.IMAGE, ContextType.FILE, ContextType.VIDEO
        ]:
            return
        ...
        #    obj = e_context["context"].kwargs['msg']._rawmsg.Text()
        #    image_storage = io.BytesIO()
        #    image_storage.write(obj)
        #    # print(e_context["context"].__dict__)
        #    # print(e_context["context"].kwargs['msg'].__dict__)
        #    # print(obj)
        #    # file_obj = open(content)
        #    reply = self.create_reply(reply_type, image_storage)
        ...
        if content == "摸鱼视频":
            moyu_video = self.get_moyu_video_calendar()
            reply_type = ReplyType.VIDEO_URL if self.is_valid_url(moyu_video) else ReplyType.TEXT
            reply = self.create_reply(reply_type, moyu_video or "摸鱼服务异常，请检查配置或者查看服务器log")
            e_context["reply"] = reply
            e_context.action = EventAction.BREAK_PASS  # 事件结束，并跳过处理context的默认逻辑
        if content == "摸鱼":
            ...
            e_context["reply"] = reply
            e_context.action = EventAction.BREAK_PASS  # 事件结束，并跳过处理context的默认逻辑
        if content.startswith("斗图"):
            keyword = content[2:].strip()
            if not self.alapi_token:
                self.handle_error("alapi_token not configured", "获取表情包请求失败")
                reply = self.create_reply(ReplyType.TEXT, "请先配置alapi的token")
            else:
                img_url = self.get_doutu(self.alapi_token, keyword)
                reply_type = ReplyType.IMAGE_URL if self.is_valid_url(img_url) else ReplyType.TEXT
                reply = self.create_reply(reply_type, img_url or "斗图服务异常，请检查配置或者查看服务器log")
                e_context["reply"] = reply
                e_context.action = EventAction.BREAK_PASS  # 事件结束，并跳过处理context的默认逻辑
        if content == "明星八卦":
            bagua = self.get_bagua_calendar()
            reply_type = ReplyType.IMAGE_URL if self.is_valid_url(bagua) else ReplyType.TEXT
            reply = self.create_reply(reply_type, bagua or "明星八卦服务异常，请检查配置或者查看服务器log")
            e_context["reply"] = reply
            e_context.action = EventAction.BREAK_PASS  # 事件结束，并跳过处理context的默认逻辑
        ...
        if content.startswith("上传"):
            if not self.params_cache.get(user_session_id):
                self.params_cache[user_session_id] = True
                reply_type = ReplyType.TEXT
                reply = self.create_reply(reply_type, '请发送文件')
                e_context["reply"] = reply
                e_context.action = EventAction.BREAK_PASS  # 事件结束，并跳过处理context的默认逻辑
                return 
        if self.params_cache.get(user_session_id) and context.type in [ContextType.FILE, ContextType.IMAGE, ContextType.VIDEO]:
            print('准备上传文件')
            context.get("msg").prepare()
            file_path = context.content
            file_url = self.upload_file(file_path)
            reply_type = ReplyType.TEXT
            reply_text = f'上传成功：{file_url}'
            del self.params_cache[user_session_id]
            reply = self.create_reply(reply_type, reply_text)
            e_context["reply"] = reply
            e_context.action = EventAction.BREAK_PASS  # 事件结束，并跳过处理context的默认逻辑
    ...
    def get_help_text(self, verbose=False, **kwargs):
        ...
        help_text += "  🐟 摸鱼: 发送“摸鱼”获取摸鱼人日历，发送“摸鱼视频”获取视频版。\n"
        help_text += "  🔥 热榜: 发送“xx热榜”查看支持的热榜。\n"
        help_text += "  🍉 明星八卦: 发送“明星八卦”查看明星八卦。\n"
        ...
        help_text += "  🥳 斗图: 发送“斗图+关键字”查表情包，如“斗图小黑子”。\n"
    ...
    def upload_file(self, file_path):
        ...
        if not cf_obj.is_valid():
            return self.handle_error('没有配置CloudFlare R2')
        file_url = cf_obj.to_r2(file_path)
        if file_url:
            return file_url
        else:
            return self.handle_error('上传失败')
    ...
    def get_moyu_video_calendar(self):
        url = "https://dayu.qqsuu.cn/moyuribaoshipin/apis.php?type=json"
        payload = "format=json"
        headers = {'Content-Type': "application/x-www-form-urlencoded"}

        try:
            moyu_video_calendar_info = self.make_request(url, method="GET", headers=headers, data=payload)
            # 验证请求是否成功
            if isinstance(moyu_video_calendar_info, dict) and moyu_video_calendar_info['code'] == 200:
                return moyu_video_calendar_info['data']
            else:
                return self.handle_error(moyu_video_calendar_info, "moyu_video_calendar请求失败")
        except Exception as e:
            return self.handle_error(e, "获取摸鱼日历视频失败")

    def get_bagua_calendar(self):
        url = 'https://dayu.qqsuu.cn/mingxingbagua/apis.php?type=json'
        headers = {'Content-Type': "application/x-www-form-urlencoded"}
        try:
            bagua_calendar_info = self.make_request(url, method="GET", headers=headers)
            # 验证请求是否成功
            if isinstance(bagua_calendar_info, dict) and bagua_calendar_info['code'] == 200:
                return bagua_calendar_info['data']
            else:
                return self.handle_error(bagua_calendar_info, "bagua_calendar请求失败")
        except Exception as e:
            return self.handle_error(e, "获取明星八卦信息失败")

    def get_doutu(self, alapi_token, keyword):
        url = BASE_URL_ALAPI + 'doutu'
        payload = f"token={alapi_token}&keyword={keyword}&page=1&type=7"
        params = {
            "token": alapi_token,
            "keyword": keyword,
            'page': 1,
        ...
        try:
            ...
            img_url = data[random.randint(0, result_count - 1)]
            return img_url
        except Exception as e:
            return self.handle_error(e, "获取表情失败")
